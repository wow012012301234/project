...
def predict():
    ...
    file_data = base64.b64decode(content['file'])

    # Process file in memory
    pe_data = io.BytesIO(file_data)
    data = extract_infos(pe_data.read())

    # Load classifier and features
    clf_path = os.path.join(os.path.dirname(
        os.path.realpath(__file__)), 'classifier/classifier.pkl')
    features_path = os.path.join(os.path.dirname(
        os.path.realpath(__file__)), 'classifier/features.pkl')

    clf = joblib.load(clf_path)
    with open(features_path, 'rb') as f:
        features = pickle.load(f)

    pe_features = [data.get(f, 0) for f in features]
